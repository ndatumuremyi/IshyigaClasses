...
public class Player extends Citizen {
    ...
    @Builder.Default
    private int yellowCardsInOneMatch = 0;
//    @Builder.Default
//    private int consecutiveYellowCards = 0;

    private int playingPosition;
    @Builder.Default
    private Boolean allowedToPlayNextMatch = true;

    private Match nearMatchPlayed;

    public void yellowCard(Match match){
        if(match.canEqual(nearMatchPlayed)){
            if (yellowCardsInOneMatch == 1) {
                redCard();
            } else {
            ...
            this.nearMatchPlayed = match;
            yellowCardsInOneMatch = 1;
        }
        totalYellowCards++;
    }
